...
async def run_anomaly_detection(company_id: str) -> dict[str, Any]:
    ...
    try:
        ...
        result = agent.invoke({"messages": [{"role": "user", "content": f"Detect anomalies in P&L data for company '{company_id}'. Use z-score and IQR methods. Flag one-time and seasonality anomalies."}]})
